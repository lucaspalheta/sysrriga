<?php
 ...
 session_start();
	...
	/*dados da area */
		...
		$a_dnls = $_POST ["DN_da_Linha_de_Succao"];
		$tf = $_POST ["Tempo_de_Funcionamento"];	
	    $a_nts = $_POST ["Numero_Total_de_Setores"];
    ...
    /*dados do solo*/
     ...
     if($proj_nome == "" && $proj_Tecnico_Responsavel == "" && $proj_CREA == "" && $proj_Proprietario == "" && $prop_nome == "" && $prop_endereco == "" && $prop_cep == "" && $prop_cidade == "" && $er == "" && $dta == "" && $ea == "" && $hf == "" && $tr == "" && $df == "" && $l ==  "" && $c == "" && $a_dnlp == "" && $a_dnll == "" && $a_clr == "" && $a_cls == "" && $a_dnlr == "" && $a_dnls == "" && $tf == "" && $a_nts == "" && $nome_cultura == "" && $data == "" && $eep == "" && $eef == "" && $as == "" && $pesr == "" && $fd == "" && $cc == "" && $ec == "" && $solo_cc == "" && $solo_pm == "" && $solo_ds == "" && $solo_irn == "" && $solo_itn == "" && $cras == "" && $ctas == "" && $vib == "" || $proj_nome == "" || $proj_Tecnico_Responsavel == "" || $proj_CREA == "" || $proj_Proprietario == "" || $prop_nome == "" || $prop_endereco == "" || $prop_cep == "" || $prop_cidade == "" || $er == "" || $dta == "" || $ea == "" || $hf == "" || $tr == "" || $df == "" || $l ==  "" || $c == "" || $a_dnlp == "" || $a_dnll == "" || $a_clr == "" || $a_cls == "" || $a_dnlr == "" || $a_dnls == "" || $tf == "" || $a_nts == "" || $nome_cultura == "" || $data == "" || $eep == "" || $eef == "" || $as == "" || $pesr == "" || $fd == "" || $cc == "" || $ec == "" || $solo_cc == "" || $solo_pm == "" || $solo_ds == "" || $solo_irn == "" || $solo_itn == "" || $cras == "" || $ctas == "" || $vib == "")  {
	...
	{ 
	  ...
	  $projeto = "INSERT INTO dados_projeto(id_usuario,id_projeto,proj_nome,proj_tecResponsavel,proj_nCrea,proj_proprietario) VALUES('$idUsuario','','$proj_nome','$proj_Tecnico_Responsavel','$proj_CREA','$proj_Proprietario')";    
       ...
       $propriedade = "INSERT INTO dados_propriedade(id_usuario,id_projeto,prop_nome,prop_endereco,prop_cep,prop_cidade) VALUES('$idUsuario','$id_projeto','$prop_nome','$prop_endereco','$prop_cep','$prop_cidade')";
	    ...
	    /* dados gerais */
       $gerais = "INSERT INTO dados_gerais(id_usuario,id_projeto,g_eto,g_dta,g_ea,g_hd,g_tr,g_df) VALUES('$idUsuario','$id_projeto','$er','$dta','$ea','$hf','$tr','$df')";
	   	    $teste3 = mysql_query($gerais)or die("Não foi possível cadastrar os dados3.");
      
	  /* dados da area */
       $area = "INSERT INTO dados_area(id_usuario,id_projeto,area_largura,area_comprimento,area_dnlp,area_dnll,area_comprimento_lr,area_comprimento_ls,area_dnlr,area_dnls,area_tf,area_nts) VALUES('$idUsuario','$id_projeto','$l','$c','$a_dnlp','$a_dnll','$a_clr','$a_cls','$a_dnlr','$a_dnls','$tf','$a_nts')";
	   	    $teste4 = mysql_query($area)or die("Não foi possível cadastrar os dados4.");			
		  
      /* dados da cultura */
       $cultura = "INSERT INTO dados_cultura(id_usuario,id_projeto,cult_nome,cult_data,cult_esp_plantas,cult_esp_fileiras,cult_area_sombreada,cult_z,cult_f,cult_kc,cult_etc) VALUES('$idUsuario','$id_projeto','$nome_cultura','$data','$eep','$eef','$as','$pesr','$fd','$cc','$ec')";
	   	    $teste5= mysql_query($cultura)or die("Não foi possível cadastrar os dados5.");	

      /* dados do solo */
       $solo = "INSERT INTO dados_solo(id_usuario,id_projeto,solo_cc,solo_pm,solo_ds,solo_irn,solo_itn,solo_cra,solo_cta,solo_vib) VALUES('$idUsuario','$id_projeto','$solo_cc','$solo_pm','$solo_ds','$solo_irn','$solo_itn','$cras','$ctas','$vib')";
